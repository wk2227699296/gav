  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" :label="t('general.operations')">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWkClassFunc(scope.row)">{{ t('general.change') }}</el-button>
...
const getTableData = async() => {
  const table = await getWkClassList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteWkClassByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateWkClassFunc = async(row) => {
    const res = await findWkClass({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rewClass
...
// 删除行
const deleteWkClassFunc = async (row) => {
    const res = await deleteWkClass({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: t('general.deleteSuccess')
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createWkClass(formData.value)
                  break
                case 'update':
                  res = await updateWkClass(formData.value)
                  break
                default:
                  res = await createWkClass(formData.value)
                  break
