  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" :label="t('general.operations')">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWkInfoFunc(scope.row)">{{ t('general.change') }}</el-button>
...
const getTableData = async() => {
  const table = await getWkInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteWkInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateWkInfoFunc = async(row) => {
    const res = await findWkInfo({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWkInfoFunc = async (row) => {
    const res = await deleteWkInfo({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: t('general.deleteSuccess')
