<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="age" prop="age" width="120" />
        <el-table-column align="left" label="gender" prop="gender" width="120">
        ...
        <el-table-column align="left" :label="t('general.operations')">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWkStudentFunc(scope.row)">{{ t('general.change') }}</el-button>
...
const getTableData = async() => {
  const table = await getWkStudentList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteWkStudentByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
...
// 更新行
const updateWkStudentFunc = async(row) => {
    const res = await findWkStudent({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWkStudentFunc = async (row) => {
    const res = await deleteWkStudent({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: t('general.deleteSuccess')
